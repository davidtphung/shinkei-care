...
export function CareApp({ onHub, onBoardChange }: Props) {
  ...
  useEffect(() => {
    if (game.screen !== 'title') headingRef.current?.focus()
  ...
  const finishRest = () => {
    ...
    setPromptName((quality.isNew || time.isNew || qualifiesForBoard('care', scored)) && scored > 0)
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute top-[max(0.75rem,env(safe-area-inset-top))] left-[max(0.75rem,env(safe-area-inset-left))] z-40">
        {running ? <RaceClock elapsed={elapsed} ink={game.screen === 'rest'} /> : null}
      </div>
      <div id="care-game">
        {game.screen === 'title' ? (
